...
// RSquare values
static double RSquare = 0.0;
static double GRSquare = 0.0;
static double MAPETrainig = 0.0; // Keep the last calculated MAPE in the training.

double  G_pBestGcv;			 // out: GCV of the best model i.e. BestSet columns of bx
int     G_pnTerms;		     // out: max term nbr in final model, after removing lin dep terms
int     G_piTermCond;		 // out: reason we terminated the foward pass
bool*   G_BestSet = NULL;    // out: nMaxTerms x 1, indices of best set of cols of bx
double* G_bx = NULL;         // out: nCases x nMaxTerms
int*    G_Dirs = NULL;       // out: nMaxTerms x nPreds, -1,0,1,2 for iTerm, iPred
double* G_Cuts = NULL;       // out: nMaxTerms x nPreds, cut for iTerm, iPred
double* G_Residuals = NULL;  // out: nCases x nResp
double* G_Betas = NULL;      // out: nMaxTerms x nResp
    ...
    const double Penalty)      // in: GCV penalty per knot
{
	...
	RSquare = 1 - RssVec[iBestModel]/RssVec[0];
	GRSquare = 1 - BestGcv/GcvNull;
    ...
    const double Trace)          // in: 0 none 1 overview 2 forward 3 pruning 4 more pruning
{
	...
	//printf("C  -> nCases = %d, nReponses = %d, nPreds = %d, nMaxDegree = %d, nMaxTerms = %d, Penalty = %f, Thresh = %f, UseBetaCache = %s, Trade = %d\n", 
	//	nCases, nResp, nPreds, nMaxDegree, nMaxTerms, Penalty, Thresh, UseBetaCache ? "true":"false", Trace);
	...
	printf("C ->\n");
    for(iCol = 0; iCol < nPreds; iCol++) {
        for(i = 0; i < min((const int)nCases, 10); i++) {
			printf("%8.4f ", x[i + iCol * nCases]);
		}
		printf("\n");
	}
	printf("\n");
	...
	*/
    ...
    ForwardPass(&nTerms, &iTermCond,
        ...
        LinPreds, AdjustEndSpan, UseBetaCache, NULL /*sPredNames*/);
    ...
    *piTermCond = iTermCond;
	...
	G_pBestGcv = *pBestGcv;
	G_pnTerms = *pnTerms;
	G_piTermCond = *piTermCond;

	if (G_BestSet != NULL)
		free(G_BestSet);
	G_BestSet = (bool *)malloc(nMaxTerms * sizeof(bool));
	memcpy(G_BestSet, BestSet, nMaxTerms * sizeof(bool));

	if (G_bx != NULL)
		free(G_bx);
	G_bx = (double *)malloc(nCases * nMaxTerms * sizeof(double));
	memcpy(G_bx, bx, nCases * nMaxTerms * sizeof(double));

	if (G_Dirs != NULL)
		free(G_Dirs);
	G_Dirs = (int *)malloc(nMaxTerms * nPreds * sizeof(int));
	memcpy(G_Dirs, Dirs, nMaxTerms * nPreds * sizeof(int));

	if (G_Cuts != NULL)
		free(G_Cuts);
	G_Cuts = (double *)malloc(nMaxTerms * nPreds * sizeof(double));
	memcpy(G_Cuts, Cuts, nMaxTerms * nPreds * sizeof(double));

	if (G_Residuals != NULL)
		free(G_Residuals);
	G_Residuals = (double *)malloc(nCases * nResp * sizeof(double));
	memcpy(G_Residuals, Residuals, nCases * nResp * sizeof(double));

	if (G_Betas != NULL)
		free(G_Betas);
	G_Betas = (double *)malloc(nMaxTerms * nResp * sizeof(double));
	memcpy(G_Betas, Betas, nMaxTerms * nResp * sizeof(double));

	MAPETrainig = 0;
	for (int iCase = 0; iCase < nCases; iCase++) {
		for (int iResponse = 0; iResponse < nResp; iResponse++)
			MAPETrainig += abs(Residuals[iCase + iResponse * nCases]) / y[iCase + iResponse * nCases];
	}
	MAPETrainig = MAPETrainig / (nCases * nResp);
...
ECOEARTH_API void PredictEarth(
    ...
    const int    nMaxTerms)
{
	//printf("C  -> nPreds = %d, nResp = %d, nTerms = %d, nMaxTerms = %d\n", 
	//	nPreds, nResp, nTerms, nMaxTerms);

    for(int iResp = 0; iResp < nResp; iResp++) {
        y[iResp] = PredictOneResponse(x, UsedCols, Dirs, Cuts,
                       &Betas[iResp * nTerms], nPreds, nTerms, nMaxTerms);

		//printf("%10.4f\t | ", y[iResp]);
		//for (int j = 0; j < nPreds; j++) {
		//	printf("%10.4f\t", x[j]);
		//}
		//printf("\n");
...
{
    for(int iResp = 0; iResp < nResp; iResp++) {
        y[iResp] = PredictOneResponse(x, G_BestSet, G_Dirs, G_Cuts,
...
ECOEARTH_API void FullPredictEarth(
    ...
    const double Trace)          // in: 0 none 1 overview 2 forward 3 pruning 4 more pruning
{
	//printf("Parameters : nCases = %d, nResp = %d, nPreds = %d, nOutputs = %d, nMaxDegree = %d, nMaxTerm = %d", nCases, nResp, nPreds, nOutputs, nMaxDegree, nMaxTerms);

	int *LinPreds		= (int *)calloc(nPreds, sizeof(int));


	double *x			= (double *)malloc(nCases    * nPreds     * sizeof(double));
	double *y			= (double *)malloc(nCases    * nResp	  * sizeof(double));
	...
	double *Residuals	= (double *)malloc(nCases    * nResp	  * sizeof(double));
	double *Betas		= (double *)malloc(nMaxTerms * nResp	  * sizeof(double));
	...
	for (int iCase = 0; iCase < nCases; iCase++) {
		for (int iPred = 0; iPred < nPreds; iPred++) {
			x[iCase + iPred * nCases] = x_train[iCase + iPred * nCases];
		}
		for (int iResponse = 0; iResponse < nResp; iResponse++)
			y[iCase + iResponse * nCases] = y_train[iCase + iResponse * nCases];
	}
	...
	Earth(&BestGcv, &nTerms, &TermCond, BestSet, bx, Dirs, Cuts, Residuals, Betas,
		...
		nCases, nResp, nPreds, nMaxDegree, nMaxTerms, Penalty, Thresh,
		nMinSpan,      // MinSpan
		nEndSpan,      // EndSpan
		Prune,   // Prune
		nFastK, FastBeta, NewVarPenalty, LinPreds, AdjuntEndSpan, true, Trace);
	...
	//for (int iResponse = 0; iResponse < nResp; iResponse++) {
		...
		double *yHat = (double *)malloc(1 * sizeof(double));

		for (int iCase = 0; iCase < nOutputs; iCase++) {
			for (int iPred = 0; iPred < nPreds; iPred++)
				xVec[iPred] = x_test[iCase + iPred * nOutputs]; // x[iCase + iPred * nOutputs];
			PredictEarth(yHat, xVec, BestSet, Dirs, Cuts, Betas, nPreds, nResp, nTerms, nMaxTerms);
			y_hat[iCase] = yHat[0];
		}
...
// Return the last GRSquare calculated
ECOEARTH_API double GetGRSquareTraining() {
	return GRSquare;
}

// Return the last MAPE calculated
